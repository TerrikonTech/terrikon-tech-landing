...
async ({ videoUrl, maskUrl, mode, frames, cols, rows, tileWidth, tileHeight,
  ...
  const sceneAtlasCanvas = document.createElement('canvas');
  sceneAtlasCanvas.width = tileWidth * cols;
  sceneAtlasCanvas.height = tileHeight * rows;
  const sceneAtlasContext = sceneAtlasCanvas.getContext('2d');
  ...
  for (let frameIndex = 0; frameIndex < frames; frameIndex += 1) {
    ...
    const destinationX = (frameIndex % cols) * tileWidth;
    const destinationY = Math.floor(frameIndex / cols) * tileHeight;
    sceneAtlasContext.drawImage(
      sourceCanvas,
      destinationX,
      destinationY,
      tileWidth,
    ...
    atlasContext.putImageData(frame, destinationX, destinationY);

    if (frameIndex === posterFrame) {
      const posterFrameData = bakeFrame(frameIndex, posterWidth, posterHeight);
      const scenePosterCanvas = document.createElement('canvas');
      scenePosterCanvas.width = posterWidth;
      scenePosterCanvas.height = posterHeight;
      scenePosterCanvas.getContext('2d').drawImage(sourceCanvas, 0, 0);
      scenePosterUrl = scenePosterCanvas.toDataURL(
        'image/webp',
        Math.min(quality + 0.06, 0.94),
      );
  ...
  return {
    ...
    sceneAtlasUrl: sceneAtlasCanvas.toDataURL('image/webp', quality),
    posterUrl,
    scenePosterUrl,
...
def bake(
    ...
    scene_atlas_path = public_dir / f"scene-{mode}-frames.webp"
    scene_poster_path = public_dir / f"scene-{mode}-poster.webp"
    ...
    scene_atlas_path.write_bytes(decode_data_url(result.pop("sceneAtlasUrl")))
    scene_poster_path.write_bytes(decode_data_url(result.pop("scenePosterUrl")))
    print(
        f"{mode}: source={result['source']} mask={result['mask']} "
        f"atlas={result['atlas']} duration={result['duration']:.3f}s "
        f"subject_atlas_bytes={atlas_path.stat().st_size} "
        f"subject_poster_bytes={poster_path.stat().st_size} "
        f"scene_atlas_bytes={scene_atlas_path.stat().st_size} "
        f"scene_poster_bytes={scene_poster_path.stat().st_size}"
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bake synchronized full-scene and transparent-subject WebP atlases."
    )
    parser.add_argument(
        "--mode",

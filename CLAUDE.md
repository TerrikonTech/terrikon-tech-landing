...
## Что это / Запуск
Русскоязычный лендинг студии разработки «Террикон Тех»: Vite + React 18 + TypeScript + Tailwind CSS 3.4 + Framer Motion 12, шрифты Unbounded (заголовки, font-display) + Golos Text (текст), акцент #FF6A00 (свитч темы, ::selection, огонёк терриконов). Секции: Hero (скраб-видео мышью, 2 темы: манекен/лама, тумблер в навбаре, liquid-glass «Связаться»; на lg фигура перекрывает заголовок: SubjectKeyCanvas — WebGL-кеинг по цвету фона рисует только субъекта поверх текста, слои видео z-0 / текст z-1 / канвас z-2, не ломать порядок), типографическая марка направлений, О нас (посимвольное проявление), Услуги (продакшн-разработка), Как работаем (4 этапа), Проекты (Space Voyage, CodeNest, Vex Ventures, Stellar AI, ASME — локальные постеры), FAQ, Команда (Олег+Савелий), гребень терриконов (SVG-горизонт TerrikonRidge), футер Террикон (контакт — только ТГ, контурный водяной знак).
...
- Первая загрузка hero — poster-first: на touch видео и WebGL-атлас включаются после первого взаимодействия; при Save-Data и reduced-motion остаётся только постер. Не возвращать блокирующий прелоадер, `preload="auto"` или мобильный autoplay.

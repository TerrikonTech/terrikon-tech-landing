...
2. **Визуальная марка** — две ленты из 21 локального WebP-постера.
...
6. **Проекты** — пять sticky-карточек: Space Voyage, CodeNest,
   Vex Ventures, Stellar AI и ASME.
...
- Шрифты, постеры и hero-медиа хранятся локально.
...
- При `prefers-reduced-motion: reduce` скраббинг, WebGL-маска, параллакс и
  длительные CSS-переходы отключаются.

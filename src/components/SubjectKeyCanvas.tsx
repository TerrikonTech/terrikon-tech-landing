...
void main() {
  ...
  if (uDark > 0.5) {
    ...
    // Внутри фигуры — сплошняк. В поясе кромки фон видео чёрный, значит
    // пиксель = свет ворса, уже «премультиплаенный» чёрным: отдаём его цвет
    // как есть (без умножения на альфу), альфа = оценка покрытия по яркости.
    // Композитор сложит: ворс светится ПОВЕРХ буквы, буква просвечивает в
    // тёмных местах — ни чёрной обводки, ни серой грязи у полутонов.
    float luma = dot(c.rgb, vec3(0.299, 0.587, 0.114));
    float solid = smoothstep(0.62, 0.85, m);
    float band = smoothstep(0.15, 0.4, m);
    gl_FragColor = vec4(c.rgb * max(solid, band),
                        max(solid, band * clamp(luma * 3.0, 0.0, 1.0)));
    return;
  } else {

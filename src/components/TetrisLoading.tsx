...
// Тетрис-лоадер (itaizeilig/tetris-loader, 21st.dev), адаптация под наш
// стек: без next-клиентских директив, цвета зафиксированы под тёмный
// оверлей прелоадера (белые фигуры на чёрном)
...
export default function TetrisLoading({
      ...
      if (linesToClear.length > 0) {
        ...
        const newGrid = prevGrid.map((row, rowIndex) => {
          if (linesToClear.includes(rowIndex)) {
            return row.map((cell) => ({
              ...cell,
              color: 'bg-white animate-pulse opacity-50',
            }))
          }
          return row
        })
      ...
      <div key={rowIndex} className="flex">
        {row.map((cell, colIndex) => (
          <div
            ...
            className={`${config.cellSize} border border-neutral-700 transition-all duration-100 ${
              cell.filled ? `${cell.color} scale-100` : 'scale-95 bg-black'
  ...
  return (
    <div>
      <div className="mb-6">
        <div
          className={`border-2 border-neutral-200 bg-black ${config.padding}`}
        >
          {renderGrid()}
        </div>
      </div>
      ...
      {showLoadingText && (
        <div className="text-center">
          <p className="font-medium text-white">{loadingText}</p>
        </div>
      )}
    </div>
  )
